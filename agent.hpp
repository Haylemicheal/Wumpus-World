...
void Agent::fill_percept_history(std::vector<std::vector<bool>> &percept_history, std::vector<bool> sense, int current_pos)
{
    ...
    percept_history.at(current_position).at(6) = true;
    if (sense.at(0) == 1)
    {
        percept_history.at(current_pos).at(3) = true;
        for (int i = 0; i < adjacent.size(); i++)
        {
            if(!percept_history.at(adjacent.at(i)).at(6))
...
void Agent::move(int &current_position,int &prev_position,std::vector<std::vector<bool>> &percept_history,int gold_cont,bool &go)
{
